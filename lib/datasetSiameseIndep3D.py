...
class DualImageIndep3DDataset(FilesArrayDataset):
    ...
    def get(self, tfm, xs, y):
        if tfm is None:
            ...
            for k,x in enumerate(xs):
                ...
                #print("Image:",k,x.shape,p_transform,tfm.tfms)
...
class FilesIndexArrayDataset3D(DualImageIndep3DDataset):
    def get_c(self): return int(self.y.max())+1


class FilesNhotArrayDataset3D(DualImageIndep3DDataset):
    @property
    def is_multi(self): return True


class FilesIndexArrayRegressionDataset3D(DualImageIndep3DDataset):
